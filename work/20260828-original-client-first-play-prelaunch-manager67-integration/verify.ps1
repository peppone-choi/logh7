...
$root=$PSScriptRoot
$tests=& (Join-Path $root 'tests/test-prelaunch-manager67-integration.ps1')|ConvertFrom-Json
$contract=& (Join-Path $root 'src/verify-prelaunch-manager67-integration.ps1') -ContractPath (Join-Path $root 'evidence/prelaunch-manager67-integration.json')|ConvertFrom-Json
...
$production=@((Join-Path $root 'verify.ps1'),(Join-Path $root 'src/verify-prelaunch-manager67-integration.ps1'))
$test=@((Join-Path $root 'tests/test-prelaunch-manager67-integration.ps1'))
...
if($tests.result-ne'PASS'-or$tests.cases-ne16-or$tests.assertions-ne28-or$contract.result-ne'PASS'-or$liveHits.Count-ne0-or$writeHits.Count-ne0){throw'aggregate verification failed'}
[ordered]@{result='PASS';tests=$tests;contract=$contract;artifactHashesVerified=$ledger.artifacts.Count;artifactLedgerSha256=(Get-FileHash -LiteralPath $ledgerPath -Algorithm SHA256).Hash;artifactHashMap=$hashMap;productionCommandCount=$productionCommands.Count;productionWriteCapabilityHits=$writeHits;testTemporaryWriteCommands=$testTempWrites;forbiddenCapabilityHits=0;validatorWrites=0;liveOperations=0;gameInputs=0;permitIssued=$false}|ConvertTo-Json -Depth 10
