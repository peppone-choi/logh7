...
'@
    ...
    Write-CanonicalJson ([ordered]@{status='STARTED';runId=$RunId;capturedAtUtc=[datetime]::UtcNow.ToString('o');helper=$helper}) $StartedPath
    ...
    $firstJson = $firstObservation | ConvertTo-Json -Depth 12 -Compress
    $secondJson = $secondObservation | ConvertTo-Json -Depth 12 -Compress
    ...
    Write-CanonicalJson ([ordered]@{status='PASS';runId=$RunId;capturedAtUtc=[datetime]::UtcNow.ToString('o');outputPath=$OutputPath;startedPath=$StartedPath;scriptSha256=$scriptHash}) $DiagnosticPath
